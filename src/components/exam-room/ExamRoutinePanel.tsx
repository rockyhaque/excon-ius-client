...
import { countBy, fmtShortDate } from "@/utils/stats";
...
type ExamRow = {
  id: string; course_code: string; course_name: string; dept: string;
  batch: string; section: string; exam_date: string; start_time: string; end_time: string; status: string;
};
...
export function ExamRoutinePanel() {
  ...
  const { data: examsRaw, isLoading, error } = useGetExamsQuery(
    semesterId ? { semester_id: semesterId, limit: 100 } : undefined,
    { skip: !semesterId },
  );
  ...
  const rows = useMemo<ExamRow[]>(
    () =>
      arr(examsRaw).map((e) => ({
        id: s(e.id), course_code: s(e.course_code), course_name: s(e.course_name), dept: s(e.dept),
        batch: s(e.batch), section: s(e.section), exam_date: s(e.exam_date).slice(0, 10),
        start_time: s(e.start_time), end_time: s(e.end_time), status: s(e.status),
      })),
    [examsRaw],
  );
  ...
  const groups = useMemo(() => {
    ...
    return [...m.entries()]
      ...
      .map(([date, list]) => [date, list.sort((a, b) => a.start_time.localeCompare(b.start_time) || a.dept.localeCompare(b.dept) || a.batch.localeCompare(b.batch))] as const);
  }, [rows]);

  // ── Semester stats & chart data ───────────────────────────────
  ...
  const examsPerDay = useMemo(() => groups.map(([date, list]) => ({ label: fmtShortDate(date), value: list.length })), [groups]);
  const deptLoad = useMemo(() => countBy(rows, (r) => r.dept, 8), [rows]);
  ...
  const openGenerate = () => {
    ...
    setNewName(""); setNewSeason("FALL"); setNewYear(2026);
    setStartDate(""); setEndDate(""); setStartTime("09:00"); setEndTime("12:00");
    setSkipWeekends(true); setReplace(false); setGenError(null);
    setGenOpen(true);
  };
  ...
  const submitGenerate = async () => {
    ...
    try {
      ...
      if (mode === "new") {
        ...
        const created = await createSemester({ name: newName.trim(), season: newSeason, year: Number(newYear) }).unwrap();
        sid = s((created as Record<string, unknown>).id);
      }
      if (!sid) return setGenError("Please choose or create a semester.");
      const res = await generate({
        semester_id: sid, start_date: startDate, end_date: endDate,
        start_time: startTime, end_time: endTime, skip_weekends: skipWeekends, replace,
      }).unwrap();
      setSemesterId(sid);
      setGenOpen(false);
  ...
  const openEdit = (r: ExamRow) => {
    setEditing(r);
    setEDate(r.exam_date); setEStart(r.start_time.slice(0, 5)); setEEnd(r.end_time.slice(0, 5));
    setEStatus(r.status || "SCHEDULED"); setEditError(null);
  };
  const submitEdit = async () => {
    ...
    try {
      await updateExam({ id: editing.id, data: { exam_date: eDate, start_time: eStart, end_time: eEnd, status: eStatus } }).unwrap();
      setEditing(null);
  ...
  const onDelete = async (r: ExamRow) => {
    if (!window.confirm(`Delete ${r.course_code} (${r.dept} B${r.batch}${r.section ? "-" + r.section : ""}) on ${r.exam_date}?`)) return;
    try {
      await deleteExam(r.id).unwrap();
  ...
  // ── PDF export (print-optimized window) ───────────────────────
  const exportPdf = () => {
    const sem = semesters.find((x) => x.id === semesterId);
    const heading = sem ? `${esc(sem.name)}` : "Exam Routine";
    const body = groups
      .map(([date, list]) => {
        const trs = list
          .map(
        ...
        return `<section><h2>${esc(fmtDay(date))}</h2>
          <table><thead><tr><th>Time</th><th>Department</th><th>Batch</th><th>Course</th><th>Status</th></tr></thead>
    ...
    <>
      <div className="foundations__toolbar">
        <div className="foundations__toolbar-left">
          <h2 className="foundations__h2" style={{ margin: 0 }}>Exam routine</h2>
          <span className="foundations__muted" style={{ margin: 0 }}>
            Auto-generate the exam timetable from foundations, then edit or export it.
          </span>
        </div>
        <div className="foundations__toolbar-right">
          <label className="foundations__field" style={{ minWidth: 180 }}>
      ...
      {!isLoading && semesterId && rows.length === 0 ? (
        <div className="foundations__empty" style={{ border: "1px solid #e5e7eb", borderRadius: 12, background: "#fff" }}>
          No exams for this semester yet. Click “Generate routine” to build the timetable.
        </div>
      ) : null}
      ...
      {rows.length > 0 ? (
        <>
          ...
          <div className="ov-grid" style={{ marginBottom: 20 }}>
            <ChartCard title="Exams per day" subtitle="Timetable density across exam days" wide>
              <AreaChart data={examsPerDay} />
            </ChartCard>
            <ChartCard title="Status mix" subtitle="Lifecycle of this semester's exams">
              <DonutChart centerLabel="exams" data={statusSlices} />
            </ChartCard>
            <ChartCard title="Department load" subtitle="Exams per department this semester">
              <HBarList data={deptLoad} unit="exams" />
            </ChartCard>
          </div>

          <p className="foundations__muted" style={{ marginTop: 0 }}>{rows.length} exam(s) across {groups.length} day(s).</p>
          {groups.map(([date, list]) => (
            <div key={date} style={{ marginBottom: 14 }}>
              <div className="routine__date" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                ...
                <span className="foundations__badge">{list.length} exam{list.length === 1 ? "" : "s"}</span>
              </div>
                  ...
                  <tbody>
                    {list.map((r) => (
                      <tr key={r.id}>
                        <td>{fmtTime(r.start_time)}–{fmtTime(r.end_time)}</td>
                        <td>{r.dept || "—"}</td>
                        <td>B{r.batch}{r.section ? `-${r.section}` : ""}</td>
                        <td><strong>{r.course_code}</strong> <span className="foundations__muted" style={{ margin: 0 }}>{r.course_name}</span></td>
                        <td>
                          <span className={`foundations__badge ${r.status === "CANCELLED" ? "foundations__badge--danger" : ""}`}>{r.status || "—"}</span>
                        </td>
                        <td>
                          <div className="foundations__actions">
                            <button type="button" className="foundations__icon-btn" onClick={() => openEdit(r)} aria-label="Edit"><IconEdit /></button>
                            <button type="button" className="foundations__icon-btn foundations__icon-btn--danger" disabled={deleting} onClick={() => void onDelete(r)} aria-label="Delete"><IconTrash /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
      ...
      <Modal
        ...
        footer={
          <div className="foundations__modal-actions">
            <button className="foundations__btn foundations__btn--ghost" type="button" onClick={() => setGenOpen(false)}>Cancel</button>
      ...
      >
        <div className="foundations__form">
          <div className="foundations__tabs" style={{ margin: 0 }}>
            <button type="button" className={`foundations__tab ${mode === "existing" ? "foundations__tab--active" : ""}`} onClick={() => setMode("existing")}>Existing semester</button>
            <button type="button" className={`foundations__tab ${mode === "new" ? "foundations__tab--active" : ""}`} onClick={() => setMode("new")}>New semester</button>
          </div>
          ...
          {mode === "existing" ? (
            <label className="foundations__field">
              ...
              <select value={genSem} onChange={(e) => setGenSem(e.target.value)}>
                ...
                {semesters.map((sm) => <option key={sm.id} value={sm.id}>{sm.name}</option>)}
              </select>
            </label>
          ) : (
            <div className="foundations__form" style={{ gap: 12 }}>
              <label className="foundations__field"><span>New semester name</span>
                <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Fall 2026" /></label>
              <label className="foundations__field"><span>Season</span>
              ...
              <label className="foundations__field"><span>Year</span>
                <input type="number" value={newYear} onChange={(e) => setNewYear(Number(e.target.value))} min={2020} max={2100} /></label>
            </div>
          )}

          <label className="foundations__field"><span>Start date</span>
            <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} /></label>
          <label className="foundations__field"><span>End date</span>
            <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} /></label>
          <label className="foundations__field"><span>Daily start time</span>
            <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} /></label>
          <label className="foundations__field"><span>Daily end time</span>
            <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} /></label>

          <label style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input type="checkbox" checked={skipWeekends} onChange={(e) => setSkipWeekends(e.target.checked)} />
            <span className="foundations__muted" style={{ margin: 0 }}>Skip weekends (Friday &amp; Saturday)</span>
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <input type="checkbox" checked={replace} onChange={(e) => setReplace(e.target.checked)} />
            <span className="foundations__muted" style={{ margin: 0 }}>Replace this semester’s existing exams</span>
          </label>
      ...
      <Modal
        ...
        footer={
          <div className="foundations__modal-actions">
            <button className="foundations__btn foundations__btn--ghost" type="button" onClick={() => setEditing(null)}>Cancel</button>
            <button className="foundations__btn" type="button" disabled={saving} onClick={() => void submitEdit()}>{saving ? "Saving…" : "Save"}</button>
          </div>
        }
      >
        {editing ? (
          <div className="foundations__form">
            <div className="foundations__muted" style={{ marginTop: 0 }}>
              {editing.course_code} · {editing.dept} · B{editing.batch}{editing.section ? `-${editing.section}` : ""}
            </div>
            <label className="foundations__field"><span>Exam date</span>
              <input type="date" value={eDate} onChange={(e) => setEDate(e.target.value)} /></label>
            <label className="foundations__field"><span>Start time</span>
              <input type="time" value={eStart} onChange={(e) => setEStart(e.target.value)} /></label>
            <label className="foundations__field"><span>End time</span>
              <input type="time" value={eEnd} onChange={(e) => setEEnd(e.target.value)} /></label>
            <label className="foundations__field"><span>Status</span>
